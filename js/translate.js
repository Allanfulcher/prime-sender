let translate_icon_1 = chrome.runtime.getURL("logo/translation_1.png");
let translate_icon_2 = chrome.runtime.getURL("logo/translation_2.png");
...
(function initVars() {
    chrome.storage.local.get(['currentLanguage', 'isTranslateMessageUsed', "lastDaySinceTranslateMessageUsed", "countOfDaysTranslateMessageUsed"], (res) => {
        currentLanguage = res.currentLanguage || 'default';
        is_translate_message_used = res.isTranslateMessageUsed || false;
        last_day_since_translate_used = res.lastDaySinceTranslateMessageUsed || "";
        count_of_days_translate_used = res.countOfDaysTranslateMessageUsed || 0;
    });
})();

chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    if (request.type === 'translate_language')
...
function replace_back_HTML_tags(translated_text, original_message) {
    ...
    Object.keys(REPLACEMENT_HTML_TAGS).forEach(key => {
        ...
        if (tag.replaceback_regex) {
            ...
            if (original_tags) {
                original_tags.forEach(original_tag => text = text.replace(tag.replacement_pattern, original_tag))
            }
...
function is_valid_translation(detected_language, targeted_language, original_text, translated_text) {
    if (!detected_language || !translated_text) return false;
    if (detected_language === targeted_language || original_text == translated_text) return false;
    let normalized_original_text = original_text.replace(/[^a-zA-Z]/g, '').toLowerCase();
    let normalized_translated_text = translated_text.replace(/[^a-zA-Z]/g, '').toLowerCase();
...
async function translate_message_HTML(original_message, targeted_language) {
    if (original_message === undefined || original_message === null || original_message.trim().length === 0)
        return "";
    let original_text = replace_HTML_tags(original_message);
    
    const translateAPI = `https://translate.googleapis.com/translate_a/single?client=gtx&sl=auto&tl=${targeted_language}&dt=t&q=${encodeURI(original_text)}`;
    try {
        ...
        if (is_valid_translation(detected_language, targeted_language, original_text, translated_text)) {
            let translated_message = replace_back_HTML_tags(translated_text, original_message);
        ...
        trackError("translate_api_contentjs_error", e);
        return [null, null];
    }
}
...
function get_detected_language_names(detected_languages_set) {
    ...
    if (size === 1)
        return [...detected_languages_set][0];
    if (size === 2)
        return [...detected_languages_set].sort().join(', ');
...
function add_traslate_button() {
    ...
    translate_div.innerHTML = `
        <div id="translate_message_btn" class="btn" style="display: ${(is_translate_enabled) ? "none" : "flex"}; text-transform:none;">
            ...
            <span class="highlight" style="text-transform:none;">Translate using Prime Sender</span>
        </div>
    ...
    let conversation_panel_wrapper = getDocumentElement('conversation_panel_wrapper');
...
function update_translate_button() {
    let targeted_language = (currentLanguage === 'default') ? 'en' : currentLanguage;
    ...
    let visible_messages_div = Array.from(all_messages_div).filter(message_div => is_in_viewport(message_div, conversation_panel));
    ...
    visible_messages_div.forEach(message_container => {
        if (message_container.querySelector('.detected_language_name')) {
            let detected_language_name = message_container.querySelector('.detected_language_name').innerText;
    ...
    let detected_language_names = get_detected_language_names(detected_languages_set);
    ...
    translate_div.querySelector('#detected_language_names').innerText = detected_language_names;
    translate_div.querySelector('#targeted_language_name').innerText = targeted_language_name;

    // Add shimmer effect to the button
    if (!is_translate_message_used && !translate_div.classList.contains('shimmer')) {
...
function replace_message(message_container) {
    if (!message_container.querySelector('.selectable-text') || !message_container.querySelector('.translated_message')) 
        return;
    let message_div = get_message_div(message_container);

    let translated_message = message_container.querySelector('.translated_message').innerHTML;
    let original_message = message_container.querySelector('.original_message').innerHTML;
    let targeted_language_code = message_container.querySelector('.targeted_language_code').innerText;
    let detected_language_code = message_container.querySelector('.detected_language_code').innerText;

    if (is_translate_enabled) {
        message_div.innerHTML = translated_message;
        message_div.setAttribute('dir', RTL_LANGUAGE_CODES.includes(targeted_language_code) ? 'rtl' : 'ltr');
    }  else if (is_translate_btn_clicked) {
        message_div.innerHTML = original_message;
        message_div.setAttribute('dir', RTL_LANGUAGE_CODES.includes(detected_language_code) ? 'rtl' : 'ltr');
    }
}
...
function translate_all_messages() {
    ...
    document.getElementById('original_message_btn').style.display = (is_translate_enabled) ? "flex" : "none";
    document.getElementById('translate_message_btn').style.display = (is_translate_enabled) ? "none" : "flex";

    // Handle translate btn click actions
    let translate_div = document.getElementById('translate_div');
    if (is_translate_enabled) {
        let today = new Date().toDateString();
        if (today == last_day_since_translate_used || count_of_days_translate_used >= 5) {
            ...
            chrome.storage.local.set({ 'lastDaySinceTranslateMessageUsed': last_day_since_translate_used, 'countOfDaysTranslateMessageUsed': count_of_days_translate_used });
        }
    }
    else {
        translate_div.classList.remove("shimmer");
...
function translate_visible_messages() {
    ...
    visible_messages_div.forEach(async (message_container, index) => {
        ...
        if (!message_container.querySelector('.translated_message_div') && message_container.querySelector('.selectable-text')) {
            ...
            if (message && message.innerText.length > 0) {
                ...
                let [detected_language, translated_message] = await translate_message_HTML(original_message, targeted_language);
                ...
                if (detected_language && translated_message) {
                    ...
                    message_container.addEventListener('click', (e) => {
                        let number_id = (e.target.dataset.jid || e.target.parentNode.dataset.jid || "");
...
function translate_messages() {
    ...
    function handleScroll(first_time = false) {
        let scroll_diff = Math.abs(conversation_panel.scrollTop - previous_scroll_pos);
        ...
        if(translate_div) {
            if(today_yesterday_div) {
                ...
                is_scrolling = setTimeout(() => translate_div.style.top = '0px', 3200);
            } else {
